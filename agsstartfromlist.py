...
def main(argv=None):
    ...
    startList = readList(args.filename)
    ...
    # Loop through each service in the list and start
    totalServices = len(startList)
    countServices = 1
    for fullSvcName in startList:
        print "Starting: " + fullSvcName + " (" + str(countServices) + " of " + str(totalServices) + ")"

        # Construct URL to get the status, then make the request
        reqURL = "/arcgis/admin/services/" + folder + fullSvcName + "/start"
        # Post the request
        try:
            data = sendRequest(args.server, args.serverport, reqURL, body, headers)
        except RequestException:
            print "Error while starting " + fullSvcName
